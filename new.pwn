...
public OnPlayerConnect(playerid)
{
    ...
    new szQuery[256];
    mysql_format(g_sql, szQuery, sizeof (szQuery), "SELECT * FROM `players` WHERE `Name` = '%e' LIMIT 1", PlayerData[playerid][pName]);
...
{
    new szQuery[256];
    mysql_format(g_sql, szQuery, sizeof (szQuery), "INSERT INTO `players` (`Name`, `Hash`) VALUES ('%e', '%s')", PlayerData[playerid][pName], hash);
